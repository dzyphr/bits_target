...
pub fn bits_target(bits: String) -> String
{
    ...
    if debug == true{println!("chopped bits: {}", chopped_bits.to_string());}
    if debug == true{println!("index: {}", index.to_string());}
    ...
    let e = BigInt::from(3);
    let step1 = &d - &e;
    if debug == true{println!("{}", step1);}
    let step2 = &c * &step1;
    if debug == true{println!("{}", step2);}
